...
- `dist/MarblePopper_v0.54/` — 展開済み公開パッケージ
- `dist/MarblePopper_v0.54.zip` — GitHub Release添付用ZIP

検証は、必須ファイル、相対参照、キャンペーン100面、JavaScript構文、マニフェストのSHA-256、ZIPのCRCと内容一致を確認します。

## 3. ローカルで最終確認する

1. `dist/MarblePopper_v0.54/index.html`を開く。
...
ゲームだけを公開する専用リポジトリなら、`dist/MarblePopper_v0.54/`の**中身**をリポジトリ直下へアップロードします。フォルダー自体を一段余計に置くとPages URLが変わります。
...
4. コミットメッセージを入力する。例: `Release Marble Popper v0.54`
...
2. 新しいタグを作る。例: `v0.54`
3. Release titleを入力する。例: `Marble Popper v0.54`
4. `CHANGELOG.md`の該当部分を説明へ貼る。
5. `dist/MarblePopper_v0.54.zip`を添付する。
