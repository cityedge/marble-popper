...
1. GitHubの **Releases** から最新の `MarblePopper_v0.42.zip` をダウンロードします。
2. ZIPをすべて展開します。
...
ビルド工程なしで `apps/marble_popper_mvp_v42.html` を開けます。配布ZIPを再構築する場合は、プロジェクトの仮想環境から次を実行します。
...
生成物は `dist/MarblePopper_v0.42/` と `dist/MarblePopper_v0.42.zip` です。
